  ...
  return (
    <div className={`fixed top-0 right-0 h-full w-80 bg-white shadow-lg z-50 transform transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
      <div className="flex justify-between items-center p-4 border-b">
        ...
        <button onClick={onClose} className="text-3xl font-bold text-gray-700 hover:text-black px-3 py-1 rounded-full focus:outline-none focus:ring-2 focus:ring-black/30 active:bg-gray-200" aria-label="Cerrar carrito">&times;</button>
      </div>
